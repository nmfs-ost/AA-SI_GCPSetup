...
def aa_show(arg):
    # If we got a command string, run it through IPython's shell so we
    # get the same behavior (and PATH inheritance, including the bin/
    # fix from 00-aa-path.py) as a top-level !!cmd.
    if isinstance(arg, str):
        try:
            ...
            ip = None
        if ip is None:
            print("aa_show: string form requires an IPython kernel.")
            return None
        out = ip.getoutput(arg)
    else:
        out = arg
