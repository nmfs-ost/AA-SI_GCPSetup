...
Usage in any notebook cell:
    ...
    aa_show(!!aa-graph file_Sv.nc --vmin -90 --vmax -30)
    aa_show(!!aa-nc --sonar_model EK60 in.raw | aa-sv | aa-clean | aa-graph)

The last stdout line of any aa-* tool is the artifact path (that's the
pipeline contract). aa_show() takes the captured output of `!!cmd`,
grabs the last line, and either displays it inline (PNG/JPG/GIF) or
prints it (any other path). The path is also returned so you can
chain further work in Python:
